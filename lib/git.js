...
function runCommand(commandName, command, params, options) {
  return new Promise((resolve, reject) => {
    ...
    ls.on("close", code => {
      if (stdout) {
        debug(`'${commandName}' stdout messages: ${stdout}`);
      }

      if (stderr) {
        debug(`'${commandName}' stderr messages: ${stderr}`);
      }
...
function commit(githubConfig) {
  ...
  return runCommand("git add", "git", ["add", CHANGES_FILE], { cwd: githubConfig.path })
    .then(runCommand.bind(null, "git commit", "git", ["commit", "-m", "\"Change\""], { cwd: githubConfig.path }));
}
